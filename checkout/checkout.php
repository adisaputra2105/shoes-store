...
<body>
  ...
  <div class="container mt-5 mb-5" style="font-family: poppins;">
    <div class="row align-items-center">
      ...
      <div class="col-md-5">
        <div class="overflow-auto px-4 py-2 rounded" style="height: 500px;">
          <?php foreach ($checkouts as $checkout) {
            $total = $checkout['harga_produk'] * $checkout['jumlah_pesanan'];
          ?>
            <div class="row px-1 py-1 bg-white border rounded-1 shadow mb-3">
              <div class="col-md-4 mt-3 mb-3"><img class="img-fluid img-responsive rounded product-image" src="../foto-produk/<?= $checkout['foto_produk'] ?>" width="100%">
              </div>
              <div class="col-md-8 ml-3">
                <h6 class="font-weight-bold mt-3"><?= $checkout['nama_produk'] ?></h6>
                <h6 class="mr-1">Harga : <b><?= number_format($checkout['harga_produk'], 0, ".", ".") ?></b></h6>
                <h6 class="mr-1">Ukuran : <b><?= $checkout['ukuran'] ?></b></h6>
                <h6 class="mr-1">Jumlah Yang Dipesan : <b><?= $checkout['jumlah_pesanan'] ?></b></h6>
                <h6 class="mr-1">Total : <b><?= number_format($total, 0, ".", ".") ?></b></h6>
              </div>
            </div>
          <?php } ?>
        </div>
      ...
      <?php $id_plg = $_SESSION['id_pelanggan'];
      $ambil = mysqli_query($koneksi, "SELECT * FROM keranjang JOIN produk ON (produk.id_produk = keranjang.id_produk)JOIN pelanggan ON (pelanggan.id_pelanggan = keranjang.id_pelanggan) WHERE keranjang.id_pelanggan = $id_plg");
        ...
        <form action="" method="POST" enctype="multipart/form-data">
          <table class="w-100" id="example">
            <tr>
              <td>Nama</td>
              <td>:</td>
              <td><input type="text" class="form-control mb-2" id="nama_pelanggan" name="nama_pelanggan" placeholder="Nama User" value="<?= $pecah['nama_pelanggan'] ?>" disabled required></td>
            </tr>
            <tr>
              <td>No.Telpon</td>
              <td>:</td>
              <td><input type="text" class="form-control mb-2" id="tlp_pelanggan" name="tlp_pelanggan" placeholder="Nomer Telepon" value="<?= $pecah['tlp_pelanggan'] ?>" disabled required></td>
            </tr>
            <tr>
              ...
              <td>
                ...
                foreach ($checkouts as $checkout) {
                  $total = $checkout['harga_produk'] * $checkout['jumlah_pesanan'];
                  $jumlah += $checkout['harga_produk'] * $checkout['jumlah_pesanan'];
                ?>
                <?php } ?>
          ...
          <!-- <a><button type="submit" class="btn btn-primary w-100 mt-4" name="bayar">Bayar</button></a> -->
